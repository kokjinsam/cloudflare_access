defmodule CloudflareAccess.JWTToken do
  @moduledoc """
  ...
  **JWT Payload Sample**
  ...
  add_hook(JokenJwks, strategy: CloudflareAccess.Certs)

  @impl Joken.Config
  def token_config do
    default_claims(skip: [:aud, :iss, :jti])
    |> add_claim("iss", nil, &(&1 == Config.jwt_iss()))
    |> add_claim("aud", nil, &(is_list(&1) && overlap?(&1, Config.jwt_aud())))
